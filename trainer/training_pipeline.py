...
def monitor_performance(output_dir: str, interval: int = 1):
    ...
    cpu_headers = [
        ...
        "memory_used_gb",
        "memory_percent",
    ] + [f"cpu_core_{i}" for i in range(psutil.cpu_count())]
    ...
    try:
        while True:
            ...
            print(
                f"[{timestamp}] GPU: {len(gpu_data)} devices, CPU: {cpu_data['cpu_percent_total']:.1f}%, Memory: {cpu_data['memory_percent']:.1f}%"
            )
...
def monitor(
    folder_name: str = typer.Option(
        "performance_logs", "--output-name", "-o", help="Output folder name"
    ),
    interval: int = typer.Option(
        1, "--interval", "-i", help="Monitoring interval in seconds"
    ),
):
    """
    ...
    console.print(
        f"[bold green]Starting background monitoring to {output_dir}[/bold green]"
    )
    console.print("Logs will be saved to:")
    console.print(
        f"  [cyan]GPU:[/cyan] {os.path.join(output_dir, 'gpu_performance.csv')}"
    )
    console.print(
        f"  [cyan]CPU:[/cyan] {os.path.join(output_dir, 'cpu_performance.csv')}"
    )
...
def training(
    config_file: str = typer.Option(
        ..., "--config", "-c", help="The path to the config file (required)"
    ),
    experiment_name: str = typer.Option(
        ..., "--experiment-name", "-e", help="The name of the experiment (required)"
    ),
    training_data: str = typer.Option(
        ..., "--train-data", "-t", help="The path to the train data (required)"
    ),
    lr: float = typer.Option(
        0.001, "--lr", "-l", help="Learning rate (default: 0.001)"
    ),
    batch_max_length: int = typer.Option(
        34, "--batch-max-length", "-b", help="Maximum batch length (default: 34)"
    ),
    workers: int = typer.Option(
        16, "--workers", "-w", help="Number of data loading workers (default: 16)"
    ),
    batch_size: int = typer.Option(
        32, "--batch-size", "-bs", help="Batch size (default: 32)"
    ),
    ...
    valInterval: int = typer.Option(
        100, "--val-interval", "-vi", help="Validation interval (default: 100)"
    ),
    FT: bool = typer.Option(True, "--FT", "-ft", help="Fine-tune the model"),
    imgH: int = typer.Option(
        64, "--img-height", "-ih", help="Image height (default: 64)"
    ),
    imgW: int = typer.Option(
        600, "--img-width", "-iw", help="Image width (default: 600)"
    ),
    sensitive: bool = typer.Option(
        True, "--sensitive", "-s", help="Use sensitive training (default: True)"
    ),
    amp: bool = typer.Option(
        False, "--amp", "-a", help="Use automatic mixed precision training"
    ),
):
    """
    ...
    with open(
        f"./saved_models/{experiment_name}/config.yaml", "w", encoding="utf8"
    ) as stream:
